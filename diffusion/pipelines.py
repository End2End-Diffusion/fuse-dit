...
class DiTPipeline(DiffusionPipeline, FromSingleFileMixin):
    ...
    def __init__(
        ...
        if hasattr(self, "vae") and self.vae is not None:
            ...
            elif hasattr(self.vae.config, "dim_mult"):
                self.vae_scale_factor = 2 ** (len(self.vae.config.dim_mult) - 1)
            else:
        ...
        if self.vae.config.latents_mean is not None and self.vae.config.latents_std is not None:
            self.shift_factor = torch.tensor(self.vae.config.latents_mean).view(1, -1, 1, 1)
            self.scaling_factor = (1 / torch.tensor(self.vae.config.latents_std)).view(1, -1, 1, 1)
        else:
            self.shift_factor = self.vae.config.shift_factor
            self.scaling_factor = self.vae.config.scaling_factor
        self.is_qwen_vae = self.vae.config._class_name == "AutoencoderKLQwenImage"
    ...
    def __call__(
        ...
        if output_type == "latent":
            ...
            # Move shift_factor and scaling_factor to latents.device
            if isinstance(self.scaling_factor, torch.Tensor) and self.scaling_factor.device != latents.device:
                self.scaling_factor = self.scaling_factor.to(latents.device)
            if isinstance(self.shift_factor, torch.Tensor) and self.shift_factor.device != latents.device:
                self.shift_factor = self.shift_factor.to(latents.device)
