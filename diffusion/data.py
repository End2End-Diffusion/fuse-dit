...
def group_by_keys_nothrow(
    ...
    for filesample in data:
        ...
        try:
            ...
            logging.warning(
                "Malformed sample, missing 'fname' or 'data' key. Skipping this sample."
            )
            continue
...
def tarfile_to_samples_nothrow(src, handler=log_and_continue):
    """Robust tarfile to samples converter that doesn't throw on errors."""
...
def expand_urls(data_path):
    """
    Expand URLs/paths handling both S3 and local paths with brace patterns.
    Supports single strings, lists, and brace expansion patterns.
    """
    
    # Convert to list if single string
    if isinstance(data_path, str):
        paths = [data_path]
    elif isinstance(data_path, (list, ListConfig)):
        paths = list(data_path)
    else:
        paths = [data_path]
    expanded = []
    for path in paths:
        expanded.extend(braceexpand.braceexpand(str(path)))
    return expanded
...
def get_llm_dataloader(hparams, *args, **kwargs):
    ...
    paths = expand_urls(hparams.data.data_path)

    dataset = wds.DataPipeline(
        wds.ResampledShards(paths, deterministic=True),
        wds.shuffle(1000, rng=random.Random(hparams.trainer.seed)),
