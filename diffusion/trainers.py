...
from transformers import CLIPTextModelWithProjection, GemmaTokenizer, CLIPTokenizer
import zstandard as zstd

from .data import get_dataloader
from .models import build_model, get_llm, update_self_attention_mask
from .pipelines import DiTPipeline, FuseDiTPipeline, FuseDiTPipelineWithCLIP
...
class Trainer(ABC):
    ...
    def dit_training_step(self, batch):
        ...
        if self.use_qwen_vae:
            model_input = model_input.unsqueeze(2)  # for `num_frames`
            model_input = self.vae.encode(model_input).latent_dist.sample()
            model_input = model_input.squeeze(2)  # back to [B, C, H, W]
        else:
            model_input = self.vae.encode(model_input).latent_dist.sample()
        model_input = (model_input - self.shift_factor) * self.scaling_factor
    ...
    def train(self):
        ...
        for step in range((self.hparams.trainer.max_steps - self.global_step) * self.hparams.trainer.gradient_accumulation_steps):
            ...
            if (step + 1) % self.hparams.trainer.gradient_accumulation_steps == 0:
                ...
                if hasattr(self.hparams.trainer, "sampling_steps") and hasattr(self.hparams, "gen") and \
                            self.global_step % self.hparams.trainer.sampling_steps == 0:
                    self.sample()
    ...
    def sample(self):
        ...
        if self.hparams.gen.model_type == "baseline-dit":
            tokenizer = GemmaTokenizer.from_pretrained(self.hparams.data.tokenizer)
            clip_tokenizer = None
            pipeline = DiTPipeline(
                transformer=unwrap(self.model),
                scheduler=scheduler,
                vae=self.vae,
                tokenizer=tokenizer,
                llm=self.llm,
            ).to(self.device)
        elif self.hparams.gen.model_type == "fuse-dit":
            tokenizer = GemmaTokenizer.from_pretrained(self.hparams.data.tokenizer)
            clip_tokenizer = None
        ...
        elif self.hparams.gen.model_type == "fuse-dit-clip":
            tokenizer = GemmaTokenizer.from_pretrained(self.hparams.data.tokenizer.llm)
            clip_tokenizer = CLIPTokenizer.from_pretrained(**self.hparams.data.tokenizer.clip)
            ...
            raise ValueError(f"Unknown model type: {self.hparams.gen.model_type}")
        pipeline.set_progress_bar_config(disable=True)

        with torch.autocast("cuda"):
            images = pipeline(
                prompt=self.hparams.gen.val_prompt,
                height=self.hparams.gen.H,
                width=self.hparams.gen.W,
                num_inference_steps=self.hparams.gen.steps,
                guidance_scale=self.hparams.gen.scale,
                num_images_per_prompt=self.hparams.gen.n_samples,
                negative_prompt=self.hparams.gen.negative_prompt or None,
                generator=torch.Generator("cpu").manual_seed(self.hparams.gen.seed),
                instruction=self.hparams.gen.instruction,
            )[0]
        ...
        free_memory()
        gc.collect()
        wandb.log({ "gen/images": [wandb.Image(image, caption=prompt) for image, prompt in zip(images, self.hparams.gen.val_prompt)] })
...
def convert_hf_to_ds_scheduler(hf_scheduler: dict, base_lr: float, total_steps: int = None):
    """
    Convert a HuggingFace-style lr_scheduler config to a DeepSpeed-compatible format.

    Args:
        hf_scheduler (dict): e.g. {"name": "constant", "num_warmup_steps": 500}
        base_lr (float): your optimizer learning rate (e.g., 3e-5)
        total_steps (int): total training steps, required for decay schedulers like cosine/linear

    Returns:
        dict: DeepSpeed lr_scheduler config
    """
    name = hf_scheduler.get("name", "constant").lower()
    warmup_steps = hf_scheduler.get("num_warmup_steps", 0)
    ...
    if name == "constant":
        ...
        # linear warmup to base_lr, then flat
        return {
            "type": "WarmupLR",
            "params": {
                "warmup_min_lr": 0.0,
...
class DeepSpeedTrainer(Trainer):
    def __init__(self, hparams, local_rank, deepspeed_config_path):
        self.hparams = hparams
        ...
        if "qwen" in self.hparams.vae.pretrained_model_name_or_path.lower():
            self.vae = AutoencoderKLQwenImage.from_pretrained(**self.hparams.vae)
            self.use_qwen_vae = True
        else:
            self.vae = AutoencoderKL.from_pretrained(**self.hparams.vae)
            self.use_qwen_vae = False
        self.vae.requires_grad_(False)
        self.vae.to(self.device)

        if self.vae.config.latents_mean is not None and self.vae.config.latents_std is not None:
            self.shift_factor = torch.tensor(self.vae.config.latents_mean).view(1, -1, 1, 1).to(self.device)
            self.scaling_factor = (1 / torch.tensor(self.vae.config.latents_std)).view(1, -1, 1, 1).to(self.device)
        else:
        ...
        ds_scheduler = convert_hf_to_ds_scheduler(
        ...
        config = {
            ...
            "optimizer": {
                "type": "AdamW",
                "params": {k: v for k, v in self.hparams.optimizer.items()}
            },
...
def get_trainer(hparams, local_rank, deepspeed_config_path):
    if ACCEL == "xla":
